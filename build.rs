use std::{collections::HashMap, fs::{read, read_to_string, File}, io::BufWriter};
...
fn generate_words_fst() {
    ...
    let words = read_to_string("data/source-words.txt")
...
fn generate_patterns_fst() {
    ...
    let mut fst = Builder::new(writer).unwrap();
    let patterns: HashMap<String, Block> = serde_json::from_slice(&read("data/preprocessed-patterns.json")
        .expect("Failed to read source patterns file")).unwrap();
    ...
    for pattern in patterns {
        ...
        fst.add(&numbered_word).expect("Failed to add pattern to FST");
    }
    
    fst.finish().expect("Failed to finish patterns FST generation");
}
