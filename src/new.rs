...
static WORDS: LazyLock<FstTree<&[u8]>> = LazyLock::new(|| {
    FstTree::from_fst(include_bytes!("words.fst"))
});

static PATTERNS: LazyLock<FstTree<&[u8]>> = LazyLock::new(|| {
    FstTree::from_fst(include_bytes!("patterns.fst"))
});
        ...
        while !remaining.is_empty() {
            ...
            if !complete {
                for i in (0..remaining.len()).rev() {
                    (new_matched, _, complete) = patterns
